...
quartier des Halles, Paris. Aucune base de données, aucun compte utilisateur.
...
| `GET /api/heatmap` | `heure` 0-23, `poids_bruit` 0-1, `poids_foule` 0-1 | FeatureCollection de LineString, `properties: {score, lden}` (≤ 8000 features) |
...
- Coordonnées hors des Halles → 400 ; la carte est bornée (`maxBounds`) pour
